...
void CGameContext::SendBroadcast(int ClientID, const char *pText, int Ticks, int Layer)
{
	...
	Broadcast.m_LastTick = Server()->Tick();
	Broadcast.m_Duration = Ticks;
	...
	if(ClientID == -1)
	{
		for(int i = 0; i < MAX_CLIENTS; i ++)
		{
			if(!m_apPlayers[i])
				continue;
			CBroadcastID BroadcastID(i, Layer);
			if(m_aBroadcasts.count(BroadcastID) && str_comp(m_aBroadcasts[BroadcastID].m_aMessage, pText) == 0)
			{
				m_aBroadcasts[BroadcastID].m_LastTick = Server()->Tick();
				m_aBroadcasts[BroadcastID] = Broadcast;
			}else
			{
				m_aBroadcasts[BroadcastID] = Broadcast;
				DoBroadcastRefresh(i);
			}
		}
		...
		Msg.m_pMessage = pText;
		Server()->SendPackMsg(&Msg, MSGFLAG_RECORD|MSGFLAG_NOSEND, ClientID);
	}
	else if(ClientID >= 0 && ClientID < MAX_CLIENTS)
	{
		CBroadcastID BroadcastID(ClientID, Layer);
		if(m_aBroadcasts.count(BroadcastID) && str_comp(m_aBroadcasts[BroadcastID].m_aMessage, pText) == 0)
		{
			m_aBroadcasts[BroadcastID].m_Duration = Server()->Tick() - m_aBroadcasts[BroadcastID].m_LastTick + Broadcast.m_Duration;
			return;
		}else
		{
			DoBroadcastRefresh(ClientID);
		}
		m_aBroadcasts[BroadcastID] = (Broadcast);
	}
}
...
void CGameContext::DoBroadcastRefresh(int ClientID)
{
	...
	bool SpaceRefresh = false, NeedFresh = false;
	for(int j = 0; j < NUM_BCLAYERS; j++)
	{
		CBroadcastID BroadcastID(ClientID, j);
		if(m_aBroadcasts.count(BroadcastID))
		{
			if(((Server()->Tick() - m_aBroadcasts[BroadcastID].m_LastTick) % Server()->TickSpeed()) == 0)
			{
				NeedFresh = true;
				break;
			}
			if((m_aBroadcasts[BroadcastID].m_LastTick + m_aBroadcasts[BroadcastID].m_Duration - Server()->Tick()) <= 0)
			{
				NeedFresh = true;
				break;
			}
	...
	if(NeedFresh)
		for(int j = 0; j < NUM_BCLAYERS; j++)
		{
			...
			if(m_aBroadcasts.count(BroadcastID))
			{
				m_aBroadcasts[BroadcastID].m_Duration = m_aBroadcasts[BroadcastID].m_LastTick + m_aBroadcasts[BroadcastID].m_Duration - Server()->Tick();
				m_aBroadcasts[BroadcastID].m_LastTick = Server()->Tick();

				if(m_aBroadcasts[BroadcastID].m_Duration > 0)
				{
					Buffer += m_aBroadcasts[BroadcastID].m_aMessage;
					AddedLayers ++;
				}
				else
				{
					m_aBroadcasts.erase(BroadcastID);
					SpaceRefresh = true;
				}
				...
				if(j == BCLAYER_BARSTART)
					Buffer.append(18 - AddedLayers, '\n');
				else Buffer += "\n"; // "\n" backspace
			}
		}

	if(!Buffer.empty() || SpaceRefresh)
	{
		CNetMsg_Sv_Broadcast Msg;
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if (MsgID == NETMSGTYPE_CL_KILL && !m_World.m_Paused)
		{
			...
			if(!m_pController->OnPlayerKillSelf(pPlayer))
				return;
...
void CGameContext::ConGiveWeapon(IConsole::IResult *pResult, void *pUserData)
{
	...
	int WeaponID = pResult->GetInteger(1);
		...
		if(ClientID == -1)
		{
			for(int i = 0; i < MAX_CLIENTS; i ++)
			{
				if(pSelf->GetPlayerChar(i))
					pSelf->GetPlayerChar(i)->GiveNinja();
			}
		}else if(ClientID >= 0 && ClientID < MAX_CLIENTS)
		{
	...
	if(ClientID == -1)
	{
		...
		{
			if(pSelf->GetPlayerChar(i))
				pSelf->GetPlayerChar(i)->GiveWeapon(WeaponID, 10);
		}
	}else if(ClientID >= 0 && ClientID < MAX_CLIENTS)
	{
		if(pSelf->GetPlayerChar(ClientID))
			pSelf->GetPlayerChar(ClientID)->GiveWeapon(WeaponID, 10);
	}
}
...
void CGameContext::OnConsoleInit()
{
	...
	Console()->Register("give_weapon", "ii", CFGFLAG_SERVER, ConGiveWeapon, this, "Give a weapon to player");

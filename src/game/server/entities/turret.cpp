...
void CTurret::Fire()
{
	...
	switch(m_Type)
	{
		case WEAPON_HAMMER:
		{
			...
			int Hits = 0;
			int Num = GameServer()->m_World.FindEntities(ProjStartPos, m_Radius*0.5f, (CEntity**)apEnts,
...
void CTurret::DoPlacer()
{
	...
	int RespawnTime = g_pData->m_aPickups[Type].m_Respawntime * (m_Type + 1) * Server()->TickSpeed() * 3 / 2;
